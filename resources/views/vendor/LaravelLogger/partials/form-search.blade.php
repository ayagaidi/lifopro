    ...
    <div class="row mb-3">
        @if(in_array('description',explode(',', config('LaravelLogger.searchFields'))))
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <input type="text" name="description" value="{{request()->get('description') ? request()->get('description'):null}}" class="form-control" placeholder="الوصف">
            </div>
        @endif
            ...
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <select class="form-control" id="user_select" name="user">
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->$userIdField }}"{{ request()->get('user') && request()->get('user') == $user->$userIdField ? ' selected':'' }}>{{ $user->username }}</option>
                    @endforeach
                </select>
            </div>
        @endif
