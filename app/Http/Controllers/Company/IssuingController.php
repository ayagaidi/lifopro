...
{
    // 1. استلام المدخلات وتنظيفها
    $plateNumber = $request->query('plate_number');
    $chassisNumber = $request->query('chassis_number');

    // 2. التحقق من وجود مدخلات للبحث
    if (empty($plateNumber) && empty($chassisNumber)) {
        return response()->json(['error' => 'يجب إدخال رقم اللوحة أو رقم الهيكل'], 400);
    }

    // 3. الحصول على بيانات الشركة والمكاتب التابعة لها
    $companyId = Auth::user()->companies_id;
    $officeIds = Office::where('companies_id', $companyId)->pluck('id')->toArray();

    // 4. بناء الاستعلام بمنطق المجموعات (Logical Grouping)
    $query = Issuing::query();

    // المجموعة الأولى: شرط الصلاحية (الشركة أو مكاتبها)
    $query->where(function ($q) use ($companyId, $officeIds) {
        $q->where('companies_id', $companyId)
          ->orWhereIn('offices_id', $officeIds);
    });

    // المجموعة الثانية: شرط البحث عن السيارة (اللوحة أو الهيكل)
    $query->where(function ($q) use ($plateNumber, $chassisNumber) {
        if ($plateNumber && $chassisNumber) {
            // إذا كتب المستخدم الإثنين، نبحث عن أي تطابق لأي منهما
            $q->where('plate_number', $plateNumber)
              ->orWhere('chassis_number', $chassisNumber);
        } elseif ($plateNumber) {
            $q->where('plate_number', $plateNumber);
        } else {
            $q->where('chassis_number', $chassisNumber);
        }
    });

    // 5. جلب آخر سجل تم إصداره بناءً على الشروط أعلاه
    $issuing = $query->with('cars')->latest('created_at')->first();
