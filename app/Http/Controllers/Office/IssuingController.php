...
class IssuingController extends Controller
{
 ...
 $todatdate=Carbon::now()->format('Y-m-d');
        $price = Price::find(1);
        return view('office.Issuing.index')
        ...
        try {
            ...
            if ($api) {
                ...
                if ($responsee->status == 2000) {
                    ...
                    $new_card = Card::where('offices_id', Auth::user()->offices_id)->where('cardstautes_id', 1)->where('card_on_hold', 0)->first();
                    ...
                    if (empty($new_card)) {
                        ...
                        $Company = Company::find(Auth::user()->offices->companies_id);
...
if ($request->insurance_days_number > $maximumDays) {
    ...
    return redirect()->back();
                        ...
                        // // $insurance_date        = date("d-m-Y H:i:s A" , strtotime($insurance_date));
                        $insurance_from = Carbon::parse($request->insurance_day_from . ' ' . $time);
                                                                                                // $insurance_to = Carbon::parse($request->nsurance_day_to . ' ' . $time);
        ...
        return redirect()->back()->withInput();
                        ...
                        if ($codeee == 7004) {
                            DB::transaction(function () use ($request, $card_id,   $insurance_day_from, $nsurance_day_to) {
                                ...
                                $issuing->office_users_id = Auth::user()->id;
                                $issuing->offices_id = Auth::user()->offices_id;
                            ...
                            if ($code == 1336) {
                                ...
                                Alert::success("تمت عملية الاصدار    بنجاح");
                                // return view('comapny.Issuing.doc')->with('ex',$ex);
                                return redirect()->route('office/document', $card_id);
                            ...
                            Alert::warning($responsee->statusMessage);

                            return redirect()->back();
                        }
                    }
                }else{
                    $new_card = Card::where('companies_id', Auth::user()->offices->companies_id)->where('cardstautes_id', 1)->where('card_on_hold', 0)->whereNull('offices_id')->first();

                    if (empty($new_card)) {

                        Alert::warning("لايوجد بطاقات مخزون الشركة قد اكتمل");
        ...
        return redirect()->back()->withInput();
                        ...
                        if ($codeee == 7004) {
                            DB::transaction(function () use ($request, $card_id,   $insurance_day_from, $nsurance_day_to) {
                                ...
                                $formatted_issuing_date = $issuing_date->format('Y-m-d H:i:s');
                                $issuing = new issuing();
                                $issuing->issuing_date = $formatted_issuing_date;
                                $issuing->insurance_name = $request->insurance_name;
                                $issuing->insurance_location = $request->insurance_location;
                                $issuing->insurance_phone = $request->insurance_phone;
                            ...
                            if ($code == 1336) {
                                ...
                                // Create a temporary PDF file on the server (optional, adjust path as needed)
                                // $tempFilePath = storage_path('app/public/' . $filename);;
                                // file_put_contents($tempFilePath, $ex);

                                // // Clear PHP output buffer (recommended for some server configurations)
                                // if (ob_get_level() > 0) {
                                //     ob_clean();
                                // }
                                ...
                                // // Read and output the PDF content
                                ...
                                // Alert::success("تمت عملية الاصدار   شركة بنجاح");
                                // return redirect()->route('company/issuing');
                                ...
                                // echo '<script>
                                //     window.open("data:application/pdf;base64,' . base64_encode($ex) . '", "_blank");
                                // </script>';
            ...
            if ($api) {
                ...
                if ($responsee->status == 2000) {
                    ...
                    if ($code == 1336) {
                        ...
                        return view('office.Issuing.doc', ['file' => asset('public/doc/' . $filename)])
                        ...
                        Alert::warning("السياسة المقابلة لرقم OC المدخل ليست في حالة معتمدة");
                        return redirect()->back();
                    } else if ($code == 8069) {
